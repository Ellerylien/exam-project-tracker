...
- **全專案進度（看板）** —— 七個階段欄位（排隊區 → 出題中 → 修改題目 → 待老師回覆 → 製作錄音稿與學生卷 → 待音檔送件 → 結案），卡片可拖曳換階段，結案時有彩帶慶祝；排隊區與出題中的卡片逾期或 3 天內截稿會亮起警示徽章，其餘已交件的卡片顯示「已交件」、不再倒數
...
- **複製專案** —— 以既有專案為範本快速建立新案
- **上傳申請表自動帶入** —— 新增專案時可上傳段考申請表（.doc／.docx，支援直接拖曳），後端用 `word-extractor` 讀檔後以固定欄位規則擷取成品名稱、審題日、老師、Email、範圍、備註等欄位自動帶入，再手動調整。審題日缺漏時，以學校收件日往回推 10 天估算；負責業務／業助依申請表上的名字對應系統人員（對照表在 `api/parse-exam-form.js`）；製作人員依登入身份預選；聽力／閱讀題型預設「照舊」。
