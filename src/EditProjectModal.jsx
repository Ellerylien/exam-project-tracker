...
export default function EditProjectModal({ isOpen, project, onClose, onProjectUpdated }) {
  ...
  return (
    <div 
      className={`fixed inset-0 bg-[#2C2A29]/40 z-[60] flex items-center justify-center p-4 backdrop-blur-sm transition-opacity duration-300 ease-out
    ...
    >
      ...
      >
        ...
        <div className="px-5 py-4 border-b border-[#EBE6DF] flex justify-between items-center bg-white shrink-0">
          <h2 className="text-lg md:text-xl font-bold text-[#4A4542]">編輯專案</h2>
        ...
        <div className="flex-1 overflow-y-auto p-5 md:p-8 bg-white">
          <form id="edit-project-form" onSubmit={handleSubmit} className="flex flex-col gap-6 md:gap-7">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-6">
              <div>
                <label className={labelClassName}>專案名稱 <span className="text-red-400">*</span></label>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-6">
              <div>
                <div className="flex justify-between items-center mb-1.5">
                  ...
                  <button type="button" onClick={() => handleCopyOld('listening_types')} className="text-xs bg-[#EBE6DF] text-[#5C544D] px-3 py-1 rounded-md shadow-[0_1px_2px_rgba(0,0,0,0.05)] border border-[#DCD6CE] hover:bg-[#DCD6CE] transition-colors font-bold">照舊</button>
                </div>
              ...
              <div>
                <div className="flex justify-between items-center mb-1.5">
                  ...
                  {/* 🔥 更新：放大按鈕、改用 MUJI 燕麥色與微陰影 */}
                  <button type="button" onClick={() => handleCopyOld('reading_types')} className="text-xs bg-[#EBE6DF] text-[#5C544D] px-3 py-1 rounded-md shadow-[0_1px_2px_rgba(0,0,0,0.05)] border border-[#DCD6CE] hover:bg-[#DCD6CE] transition-colors font-bold">照舊</button>
                </div>
        ...
        <div className="px-5 py-4 border-t border-[#EBE6DF] bg-[#FAF9F6] flex justify-end gap-3 shrink-0">
          <button type="button" onClick={onClose} className="px-5 py-2 md:py-2.5 text-sm text-[#635B56] bg-white border border-[#EBE6DF] hover:bg-[#F7F5F0] rounded-md font-bold transition-colors">取消</button>
          <button type="submit" form="edit-project-form" disabled={isSubmitting} className="px-5 py-2 md:py-2.5 text-sm bg-[#5C544D] text-white rounded-md font-bold hover:bg-[#4A433D] transition-colors flex items-center gap-2 shadow-sm disabled:opacity-50">
