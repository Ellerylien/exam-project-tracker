      ...
      if (project.has_unread) {
        supabase.from('projects').update({ has_unread: false }).eq('id', project.id).then(({ error }) => {
          if (!error && onStatusChange) {
            onStatusChange(project.id, project.status, false); 
          }
        });
      }
    }
...
<button onClick={onClose} className="text-gray-400 hover:text-gray-700 bg-gray-200/50 hover:bg-gray-200 p-1.5 rounded-full transition-colors shrink-0">
  <svg className="w-5 h-5 md:w-6 md:h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12"></path></svg>
            ...
            <div className="flex items-center gap-2 w-full">
              ...
              <button onClick={() => setIsEditModalOpen(true)} className="text-xs md:text-sm bg-blue-50 text-blue-600 hover:bg-blue-100 px-3 md:px-4 py-1.5 rounded-lg font-medium transition-colors border border-blue-200">
                修改
              </button>
          ...
          <div className="flex-1 overflow-y-auto p-4 md:p-6 flex flex-col gap-4 md:gap-6">
            ...
            <div className="bg-gray-50 p-4 rounded-xl border border-gray-100 flex flex-col gap-3 text-sm">
              ...
              <div>
                ...
                <div onClick={handleCopyEmail} className="group flex items-center gap-2 cursor-pointer w-max">
                  <span className="font-semibold text-blue-600 group-hover:text-blue-800 transition-colors break-all">{project.teacher_email || '無'}</span>
            ...
            {project.status === '待老師回覆' && (
              <div className="bg-gray-50 border border-gray-200 p-4 rounded-xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 shadow-sm">
                <div><p className="font-bold text-gray-800">老師回覆處理</p><p className="text-xs md:text-sm text-gray-500 mt-1">請根據老師的回饋，選擇對應的處理方式。</p></div>
                <div className="flex gap-2 w-full sm:w-auto">
                  <button onClick={handleNeedsRevision} className="flex-1 sm:flex-none bg-white hover:bg-red-50 text-red-600 border border-red-200 px-4 py-2 rounded-lg font-medium shadow-sm text-center">需修改</button>
                  <button onClick={handleClientApproval} className="flex-1 sm:flex-none bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg font-medium shadow-sm flex items-center justify-center gap-1">無誤</button>
                </div>
              </div>
            )}

            {['製作錄音稿與學生卷', '待音檔送件'].includes(project.status) && (
              <div className="bg-blue-50 border border-blue-200 p-4 rounded-xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 shadow-sm animate-fade-in">
                <div>
                  <p className="font-bold text-blue-800">專案結案審查</p>
                  <p className="text-xs md:text-sm text-blue-600 mt-1">目前進度已符合結案規範。若已全數確認完畢，可直接進行結案存檔。</p>
                </div>
                <button onClick={handleCloseProject} className="w-full sm:w-auto bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg font-medium transition-colors shadow-sm flex items-center justify-center gap-1">
                  🎉 辦理結案
                </button>
              </div>
            )}
            ...
            <div className="flex flex-col gap-3 md:gap-4">
              <h3 className="font-bold text-gray-800 border-b pb-2 text-sm md:text-base">討論區</h3>
                  ...
                  {topLevelComments.map(comment => {
                    ...
                    return (
                      <div key={comment.id} className="flex flex-col gap-2">
                        <div className={`flex gap-2 md:gap-3 p-3 rounded-xl border ${isSystem ? 'bg-yellow-50 border-yellow-100' : 'bg-white border-gray-100 shadow-sm'}`}>
                          <div className={`w-7 h-7 md:w-8 md:h-8 shrink-0 rounded-full flex items-center justify-center font-bold text-xs md:text-sm ${isSystem ? 'bg-yellow-100 text-yellow-600' : 'bg-blue-100 text-blue-600'}`}>
                            {getAvatarInitial(comment.author)}
                          </div>
                          
                          <div className="flex-1 min-w-0">
                            <div className="flex justify-between items-baseline mb-0.5">
                              <span className={`font-bold text-xs md:text-sm ${isSystem ? 'text-yellow-800' : 'text-gray-800'}`}>{comment.author}</span>
                              <span className="text-[10px] md:text-xs text-gray-400">{new Date(comment.created_at).toLocaleString('zh-TW', { month:'short', day:'numeric', hour:'2-digit', minute:'2-digit' })}</span>
                            </div>
                        ...
                        {replies.length > 0 && (
                          <div className="flex flex-col gap-2 ml-6 md:ml-10 pl-2 md:pl-3 border-l-2 border-gray-100">
                            {replies.map(reply => (
                              <div key={reply.id} className="flex gap-2 p-2 rounded-lg bg-gray-50 border border-gray-100">
                                <div className="w-5 h-5 md:w-6 md:h-6 shrink-0 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center font-bold text-[10px] md:text-xs">
                                  {getAvatarInitial(reply.author)}
                                </div>
                                <div className="flex-1 min-w-0">
                                  <div className="flex justify-between items-baseline mb-0.5">
                                    <span className="font-bold text-[11px] md:text-xs text-gray-700">{reply.author}</span>
                                    <span className="text-[9px] md:text-[10px] text-gray-400">{new Date(reply.created_at).toLocaleString('zh-TW', { hour:'2-digit', minute:'2-digit' })}</span>
                                  </div>
          ...
          <div className="p-3 md:p-4 border-t border-gray-100 bg-white flex flex-col gap-2">
            ...
            <div className="flex flex-col sm:flex-row gap-2">
              <select value={currentUser} onChange={handleUserChange} className="w-full sm:w-[110px] px-2 py-2 border border-gray-200 rounded-lg outline-none text-sm font-medium text-gray-700 bg-gray-50 shrink-0">
                {TEAM_MEMBERS.map(name => <option key={name} value={name}>{name}</option>)}
              </select>
              <div className="flex gap-2 flex-1 w-full">
                <input type="text" value={newComment} onChange={(e) => setNewComment(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleAddComment()} placeholder={replyingTo ? "輸入回覆..." : "新增討論..."} className="flex-1 px-3 py-2 border border-gray-200 rounded-lg outline-none text-sm min-w-0" />
                <button onClick={handleAddComment} className="px-4 md:px-5 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700 transition-colors shrink-0">送出</button>
              </div>
            </div>
          </div>
